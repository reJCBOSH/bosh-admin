...
var InitSchema = &gormigrate.Migration{
	...
	Migrate: func(tx *gorm.DB) error {
		...
		defaultPwd, _ := util.BcryptHash(global.DefaultPassword)
		user := model.SysUser{
			Username:     global.SuperAdmin,
			Password:     defaultPwd,
			PwdUpdatedAt: db.CustomTime(time.Now()),
			Nickname:     "超级管理员",
			Gender:       0,
			Introduce:    "行天之道，总司一切",
			Status:       1,
		...
		menus := []model.SysMenu{
			...
			{Path: "user", Name: "SystemUser", Component: "/system/user/index", ParentId: 1, Title: "用户管理", Icon: "ri:user-line", KeepAlive: true},
